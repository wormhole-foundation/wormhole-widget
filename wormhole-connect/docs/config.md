# Wormhole Connect - more `config` examples
...
You can provide a whitelist of routes under `routes`. By default, Connect will offer its full built-in list:
...
```json
{
  ...
  "moreNetworks": {
    ...
    "target": "_blank",
    "description": "brief description that should be displayed as tooltip when the user over a more network icon",
    "networks": [
      {
...
Enable explorer button to allow users to search for their transactions on a given explorer filtering by their wallet address
